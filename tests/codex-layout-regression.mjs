...
const combination = rule(".codex-combination");
assert.match(combination, /position:\s*relative/);
assert.match(combination, /display:\s*flex/);
assert.match(combination, /flex-direction:\s*column/);
assert.match(combination, /overflow:\s*hidden/);

const preview = rule(".codex-preview");
assert.match(preview, /height:\s*180px/);
assert.match(preview, /min-height:\s*180px/);
assert.match(preview, /max-height:\s*180px/);
assert.match(preview, /flex:\s*0 0 180px/);
...
assert.ok(
  js.indexOf('class="codex-preview"') < js.indexOf('class="codex-recipe-summary"') &&
    js.indexOf('class="codex-recipe-summary"') < js.indexOf("<dl>") &&
    js.indexOf("<dl>") < js.indexOf('class="codex-specials"'),
