...
assert.deepEqual({...maps.ORIGINAL_S.spawn},{x:44.8,y:86.2}); assert.deepEqual({...maps.ORIGINAL_S.destination},{x:65.2,y:6.2});
assert.deepEqual({...maps.CURVED_MAP.spawn},{x:73.2,y:8.2}); assert.deepEqual({...maps.CURVED_MAP.destination},{x:47.2,y:84.5});
assert.deepEqual({...maps.LOOP_MAP.spawn},{x:40.7,y:85.2}); assert.deepEqual({...maps.LOOP_MAP.destination},{x:72.9,y:8.2});
...
assert.equal(maps.LOOP_MAP.route.length,7); assert.match(source,/ROUTE_CACHES/); assert.match(source,/binary|while \(low < high\)/);
