...
const GAME_VERSION = "1.15.6 BETA";
...
function bootstrapGame() {
  ...
  if(mainMenuToggle&&mainMenuDropdown){
    let lastMenuTouch=0;
    const toggleMainDropdown=(event)=>{event?.preventDefault?.();event?.stopPropagation?.();const opening=mainMenuDropdown.hidden;mainMenuDropdown.hidden=!opening;mainMenuToggle.setAttribute("aria-expanded",String(opening));};
    // Use native touch/click here rather than the shared gesture helper: Safari
    // can dispatch both pointer and touch streams for this fixed top-layer control.
    mainMenuToggle.addEventListener("touchend",(event)=>{lastMenuTouch=Date.now();toggleMainDropdown(event);},{passive:false});
    mainMenuToggle.addEventListener("click",(event)=>{if(Date.now()-lastMenuTouch<700){event.preventDefault();event.stopPropagation();return;}toggleMainDropdown(event);});
    mainMenuDropdown.addEventListener("touchend",(event)=>event.stopPropagation(),{passive:true});
    mainMenuDropdown.addEventListener("click",(event)=>event.stopPropagation());
    document.addEventListener("pointerdown",(event)=>{if(!mainMenuDropdown.hidden&&!event.target.closest(".main-menu-dropdown-wrap")){mainMenuDropdown.hidden=true;mainMenuToggle.setAttribute("aria-expanded","false");}});
  }
