...
const GAME_VERSION = "1.15.4 BETA";
...
function bootstrapGame() {
  ...
  if(mainMenuToggle&&mainMenuDropdown){const toggleMainDropdown=(event)=>{event?.preventDefault?.();event?.stopPropagation?.();const opening=mainMenuDropdown.hidden;mainMenuDropdown.hidden=!opening;mainMenuToggle.setAttribute("aria-expanded",String(opening));};bindPointerTap(mainMenuToggle,toggleMainDropdown);mainMenuToggle.addEventListener("touchend",(event)=>{if(event.defaultPrevented)return;toggleMainDropdown(event);},{passive:false});}
