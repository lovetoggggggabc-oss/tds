  ...
  static renderCodex() {
    zodiacCodexList.innerHTML = Object.entries(ZODIAC_RECIPES)
      .map(([definitionId, zodiac]) => {
        ...
        return `<article class="zodiac-card ${definitionId.toLowerCase()} ${discovered ? "discovered" : "undiscovered"}" data-constellation="${definitionId}"><h3>${zodiac.name}</h3><section class="codex-combination" aria-label="필요한 별 조합: ${summary}"><h4>STAR RECIPE</h4><svg class="codex-preview" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${zodiac.name} 별자리 연결 그림"><g class="codex-edges">${edges}</g><g class="codex-nodes">${stars}</g></svg><div class="codex-recipe-summary"><span>필요한 별</span>${summary}</div></section><dl><div><dt>공격력</dt><dd>${zodiac.attackDamage}</dd></div><div><dt>공격속도</dt><dd>${zodiac.attackSpeed}회/초</dd></div><div><dt>사거리</dt><dd>${zodiac.range}</dd></div></dl><div class="codex-specials">${abilities}</div></article>`;
      })
      .join("");
  }
