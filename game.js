...
const GAME_VERSION = "1.12.2 BETA";
...
function bootstrapGame() {
  ...
  if(signupButton)bindPointerTap(signupButton,(event)=>{event.stopPropagation();runSignup();});
  if(loginButton)bindPointerTap(loginButton,(event)=>{event.stopPropagation();runLogin();});
  if(logoutButton)bindPointerTap(logoutButton,async(event)=>{event.stopPropagation();await pushCloudSave();await supabaseClient?.auth.signOut();authUser=null;refreshAccountUI();setAccountStatus("로그아웃했습니다.");});
  window.addEventListener("astra-auth-updated",()=>{refreshAccountUI();updateMetaCurrency();});
  if(supabaseClient){supabaseClient.auth.getSession().then(async({data})=>{if(data.session?.user)await syncCloudAfterLogin(data.session.user);refreshAccountUI();}).catch(()=>setAccountStatus("로그인 상태를 확인하지 못했습니다."));supabaseClient.auth.onAuthStateChange((event)=>{if(event==="SIGNED_OUT"){authUser=null;refreshAccountUI();}});}else setAccountStatus("로그인 서버 스크립트를 불러오지 못했습니다.");
